...
bandconvelease <- dat[dat$release_year == "",]

summaryrelease <- bandconvelease %>% group_by(release_year, cultivar_name) %>% 
  summarise(count = n())
write.csv(summaryrelease, "output/bandconveleasedate.csv")

dat$cultivar_release <- str_sub(dat$release_year,1,4)

summarycult <- dat %>% group_by(year, cultivar_name) %>%
                summarise(count = n()) # check if cultivar has more than one observation peryear
...
dat <- dat[,c("year", "region", "provence", "locality", "locality_no", "cultivar_name", "alternate", "cultivar_no", "cultivar_br", "rep", "yield", "color", "technology", "Bt", "crop", "temp", "irrigated","land_type", "number", "release_year")]
...
dat <- dat[,c("year", "region", "provence", "locality", "locality_no", "cultivar_name", "rep", "yield", "color", "technology", "Bt", "crop", "temp", "irrigated", "release_year")]
...
btsummarystat4 <- bandconv  %>%
    group_by(technology, provence) %>%
      count()

btsummarystat <- merge(btsummarystat, btsummarystat3, by = c("provence","technology"))
names(btsummarystat)[names(btsummarystat) == "n"] <- "num_of_cultivars"
names(btsummarystat4)[names(btsummarystat4) == "n"] <- "num_of_observations"
btsummarystat <- merge(btsummarystat, btsummarystat4, by = "provence", "technology")

btsummarystat <- btsummarystat %>%
            gt(rowname_col = "province") %>%
  tab_header(
    title = "Characteristics of Provinces",
    subtitle = glue::glue("observations included in sample")
  ) %>%
  cols_label(
    provence = html("Province"),
    yield = html("Average Yield"),
    min = html("Min Year"),
    max = html("Max Year"),
...
sumrel <- dat %>% filter(technology == "B") %>% group_by(technology, release_year) %>% summarise(count = n())
write.csv(sumrel, "brelease.csv")


dat$release_year[is.na(dat$release_year)] <- ""


bandconvelease2 <- prov[prov$release_year == "",]

summaryrelease2 <- prov %>% filter(technology == "B") %>% group_by(year,release_year,technology, cultivar_name) %>% 
  summarise(count = n())
summaryrelease2 <- summaryrelease2[summaryrelease2$technology == "B",]
write.csv(summaryrelease2, "bandconveleaseb4.csv")
