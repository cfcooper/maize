...
noR <- dat[!(dat$technology == "R"),]
#conventional summ stat
convsummarystat <- noR %>%
  filter(technology== "conv") %>%
  group_by(color, technology, provence, .add = FALSE) %>% 
            summarise(mean = mean(yield, na.rm = T), 
                      SD = sd(yield, na.rm = T), 
                      max = max(yield, na.rm = T), 
                      min = min(yield, na.rm = T))

convsummarystat <- convsummarystat %>%
...
print(convsummarystat)
  
  

#bt summ stat
btsummarystat <- noR %>%
  filter(technology== "B"| technology == "BR") %>%
  group_by(color, technology, provence, year, .add = FALSE) %>% 
            summarise(mean = mean(yield, na.rm = T), 
                      SD = sd(yield, na.rm = T), 
                      max = max(yield, na.rm = T), 
                      min = min(yield, na.rm = T))
...
norelease2 <- prov[prov$release_year == "",]
...
write.csv(summaryrelease2, "noreleaseb4.csv")
