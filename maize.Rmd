...
# check cultivar if in more than one obersvation per provence/year
...
btsummarystat <- merge(btsummarystat, btlocal, by = c("provence","technology"))
names(btsummarystat)[names(btsummarystat) == "n"] <- "num_of_localities"
...
write.csv(btsummarystat,"btsummarystat.csv")
...
summaryrelease <- bandconvelease %>% group_by(release_year, cultivar_name) %>% summarise(count = n())
