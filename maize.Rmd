...
write.csv(cultivar,"data/cultivarbackup.csv") 

mergeall <- bind_rows(olddat,dat)

mergeall$locality <- if_else(mergeall$locality %in% 
                               c("bethelhem", "bethelehem"), 
                             "bethlehem", mergeall$locality) #fix locality names in merged df
mergeall$locality <- if_else(mergeall$locality %in% 
                               c("bloemkomspruit"), 
                             "bloekomspruit", mergeall$locality)
mergeall$locality <- if_else(mergeall$locality %in% 
                               c(" coligny"), 
                             "coligny", mergeall$locality)
mergeall$locality <- if_else(mergeall$locality %in% 
                               c(" wesselsbron"), "wesselsbron", mergeall$locality)
mergeall$locality <- if_else(mergeall$locality %in% 
                               c("foiuriesburg"), "fouriesburg", mergeall$locality)
mergeall$locality <- if_else(mergeall$locality %in% 
                               c("grooptpan"), 
                             "grootpan", mergeall$locality)
mergeall$locality <- if_else(mergeall$locality %in% 
                               c("kapsteeel"), "kapsteel", mergeall$locality)
...
summarycult <- mergeall %>% group_by(year, cultivar_name) %>%
...
summaryloc <- mergeall %>% group_by(year, locality) %>% 
  summarise(count = n()) # check location if in more than one year
summaryloc2 <- summaryloc %>% group_by(locality) %>% 
...
mergeall <- mergeall[,c("year", "region", "provence", "locality", "locality_no", "cultivar_name", "alternate", "cultivar_no", "cultivar_br", "rep", "yield", "color", "technology", "Bt", "crop", "temp", "irrigated", "number", "release_year", "land_type")]


rm(summaryloc, summaryloc2, summarylocall, summarylocall2, summarylocall3, summarycult, summarycult2,mergedat)

mergeall$Bt <- ifelse(mergeall$technology == "B" | 
                      mergeall$technology == "BR", "Bt", "Conv")
...
write.csv(mergeall,"data/allyearsmerge.csv") 
