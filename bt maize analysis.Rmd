...
modelregress <- msummary(models,coef_map = reduced, vcov = vcovHC, stars = TRUE)
print(modelregress)
...
ZAF <- gadm_sp_loadCountries('ZAF', level=1, basefile="./")
gadm_saveStripped(ZAF, "SA")

ZAF2 <- gadm_loadStripped("SA", level=1)

map <- gadm_sp_loadCountries(c("NAM","BWA","ZWE","MOZ","LSO"), level=0, basefile="./")
gadm_plot(ZAF)
saveAs(ZAF, "output/SA")
saveAs(map, "output/MAP")

library(sf)
  SA <- st_read("gadm40_ZAF_1.shp")


file <-  readRDS("gadm36_ZAF_1_sp.rds")
file2 <- readRDS("output/MAP_adm0.rds")
...
ggsa <- ggplot(data = file, aes(x = long, y = lat)) +
