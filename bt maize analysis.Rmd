...
p_load(sandwich,
       lmtest,
       gtsummary)
install.packages("RColorBrewer","kableExtra", "gt")
...
reg2 <- glm(yield ~ provence + factor(year)+ GM + year*GM*color + yearsq*GM*color + color + irrigated, data=dat)
summary(reg2)


dat$y_effect2 <- reg2$coefficients["GM"] + reg2$coefficients["color:GM"]*dat$year + reg2$coefficients["GM:year"] * dat$year
dat$ysq_effect2 <- reg2$coefficients["GM"] + reg2$coefficients["coloryellow"] + reg2$coefficients["GM:year"] * dat$year + reg2$coefficients["GM:yearsq"] * dat$yearsq
max(dat$ysq_effect2)
...
modelregress <- modelsummary(models,output = "modelregress.png", vcov = vcovHC, stars = TRUE)
print(modelregress)
...
prov2 <- rbind(EC, FS, GP, MP, NW, KZN, NC, WC)
